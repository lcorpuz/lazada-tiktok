@component()
export class HookController extends APJS.BasicScriptComponent {

	private isMoving: boolean = false;
	private isReturning: boolean = false;
	private startWorldY: number = 0;
	private currentLocalY: number = 0;
	private dropSpeed: number = 15;
	private maxDrop: number = 25;
	private hookTransform: APJS.Transform | null = null;
	private rodTransform: APJS.Transform | null = null;
	private orbSpawner: any = null;
	private score: number = 0;

	// Catch thresholds in world space
	private catchThresholdX: number = 0.7;
	private catchThresholdY: number = 0.7;

	// Timer
	private timeLeft: number = 20;
	private gameOver: boolean = false;
	private gameOverPanel: APJS.SceneObject | null = null;
	private gameOverText: APJS.SceneObject | null = null;
	private playAgainButton: APJS.SceneObject | null = null;
	private playAgainText: APJS.SceneObject | null = null;
	...
	private getHookWorldPosition(): { x: number, y: number } {
	...
	private checkCatch() {
		if (!this.hookTransform || !this.orbSpawner) return;
	...
	private updateScoreText() {
		...
		if (obj) {
			const t = obj.getComponent("Text") as any;
			if (t) t.text = "Score: " + this.score;
		}
	}
	...
	private updateTimerText() {
		...
		if (obj) {
			const t = obj.getComponent("Text") as any;
			if (t) t.text = "Time: " + Math.ceil(this.timeLeft);
		}
	}

	private endGame() {
		this.gameOver = true;
		if (this.orbSpawner) this.orbSpawner.gameOver = true;
		if (this.gameOverPanel?.enabled === false) this.gameOverPanel.enabled = true;
		...
		if (this.orbSpawner) {
			const orbs = [...this.orbSpawner.getActiveOrbs()];
			for (const orb of orbs) orb.enabled = false;
		}
		...
		if (this.gameOverText) {
			const t = this.gameOverText.getComponent("Text") as any;
			if (t) t.text = "GAME OVER\nScore: " + this.score;
		}
	...
	private setPlayAgainVisible(isVisible: boolean) {
	...
	private setGameOverTextVisible(isVisible: boolean) {
		if (this.gameOverText) {
	...
	onUpdate(deltaTime: number) {
		...
		if (!this.isMoving || !this.hookTransform) return;
